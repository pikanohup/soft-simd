...
const { DEFAULT_TASK_TIMEOUT, DEFAULT_POOL_OPTION, DEFAULT_BROWSER_OPTION } = require('./constants')
...
class PuppeteerPool extends AsyncClass {
  constructor (poolOption = DEFAULT_POOL_OPTION, browserOption = DEFAULT_BROWSER_OPTION) {
    super(async () => {
      const { maxUseCount } = poolOption
      const factory = {
        create: async () => {
      ...
      delete poolOption.maxUseCount
      const pool = await genericPool.createPool(factory, poolOption)
      this._pool = pool
    })
  }
